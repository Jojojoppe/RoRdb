...
// Recursively generate tree structure
// Used by db_get_locations
function db_get_locations_generate_tree($childarray, $node_id, $node){
	// node contains the current node
	$ret = array();
	...
	foreach($childarray[$node_id]["childids"] as $childid){
		$child = $childarray[$childid];
		array_push($ret["childids"], $childid);
		$childret = db_get_locations_generate_tree($childarray, $childid, $child["name"]);
		array_push($ret["childs"], $childret);
		// Add all flatten nodes
		foreach($childret["flatten"] as $i) array_push($ret["flatten"], $i);
		foreach($childret["flattenids"] as $i) array_push($ret["flattenids"], $i);
...
// Returns array with all locations
// subcatecogies are places as sub-arrays
function db_get_locations(){
	GLOBAL $database_url;
	GLOBAL $locations_tab_name;
	GLOBAL $locations_col_ID;
	GLOBAL $locations_col_NAME;
	GLOBAL $locations_col_PARENT;
	GLOBAL $locations_col_PARENTID;
	GLOBAL $locations_col_PARENT_LIST;
	GLOBAL $locations_col_PARENTID_LIST;
	GLOBAL $locations_col_CHILD_LIST;
	GLOBAL $locations_col_CHILDID_LIST;

	// Get relevant information from locations tab
	$query = "select $locations_col_ID,$locations_col_NAME,$locations_col_CHILD_LIST,$locations_col_CHILDID_LIST,$locations_col_PARENTID";
	$res = dbLookup($query, $database_url, $locations_tab_name);
	if(db_check_error($res)){
		return FALSE;
	}
	...
	// Generate full tree
	foreach($rootnodes as $root_id=>$root_name){
		array_push($fulltree, db_get_locations_generate_tree($childarray, $root_id, $root_name["name"]));
	}
...
function db_get_items_from_category($category){
	...
	GLOBAL $items_col_LOCATION;

	// Get relevant information from items tab where $category (ID!) can be found in the search list
	$query = "select $items_col_ID,$items_col_NAME,$items_col_CATEGORY,$items_col_CATEGORYID,$items_col_CATEGORY_SEARCH_LIST_ID,$items_col_HIDDEN,$items_col_LOCATION ".
	...
	foreach($table["rows"] as $rowarray){
		...
		$row_LOCATION = $rowarray["c"][6]["v"];

		$item = array();
		$item["id"] = $row_ID;
		$item["name"] = $row_NAME;
		$item["category"] = $row_CATEGORY;
		$item["location"] = $row_LOCATION;
...
function db_get_items_from_location($location){
	GLOBAL $database_url;
	GLOBAL $items_tab_name;
	GLOBAL $items_col_ID;
	GLOBAL $items_col_NAME;
	GLOBAL $items_col_LOCATION;
	GLOBAL $items_col_LOCATIONID;
	GLOBAL $items_col_LOCATION_SEARCH_LIST;
	GLOBAL $items_col_LOCATION_SEARCH_LIST_ID;
	GLOBAL $items_col_HIDDEN;
	GLOBAL $items_col_CATEGORY;

	// Get relevant information from items tab where $location (ID!) can be found in the search list
	$query = "select $items_col_ID,$items_col_NAME,$items_col_LOCATION,$items_col_LOCATIONID,$items_col_LOCATION_SEARCH_LIST_ID,$items_col_HIDDEN,$items_col_CATEGORY ".
		"where ($items_col_LOCATION_SEARCH_LIST_ID contains ',".$location.",') and ".
		"($items_col_HIDDEN=0)";
	echo $query."<br>";
	$res = dbLookup($query, $database_url, $items_tab_name);
	if(db_check_error($res)){
	...
	foreach($table["rows"] as $rowarray){
		$row_ID = $rowarray["c"][0]["v"];
		$row_NAME = $rowarray["c"][1]["v"];
		$row_LOCATION = $rowarray["c"][2]["v"];
		$row_CATEGORY = $rowarray["c"][6]["v"];
