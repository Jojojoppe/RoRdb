 ...
 */
namespace RoRdb\Google\Auth;

use DateTime;
use Exception;
use RoRdb\Firebase\JWT\ExpiredException;
use RoRdb\Firebase\JWT\JWT;
use RoRdb\Firebase\JWT\SignatureInvalidException;
use RoRdb\Google\Auth\Cache\MemoryCacheItemPool;
use RoRdb\Google\Auth\HttpHandler\HttpClientCache;
use RoRdb\Google\Auth\HttpHandler\HttpHandlerFactory;
use RoRdb\GuzzleHttp\Psr7\Request;
use RoRdb\GuzzleHttp\Psr7\Utils;
use InvalidArgumentException;
use RoRdb\phpseclib\Crypt\RSA;
use RoRdb\phpseclib\Math\BigInteger;
use RoRdb\Psr\Cache\CacheItemPoolInterface;
use RuntimeException;
use RoRdb\SimpleJWT\InvalidTokenException;
use RoRdb\SimpleJWT\JWT as SimpleJWT;
use RoRdb\SimpleJWT\Keys\KeyFactory;
use RoRdb\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
    ...
    public function verify($token, array $options = [])
    {
        ...
        try {
            ...
            // firebase/php-jwt 3+
        } catch (\RoRdb\ExpiredException $e) {
            // firebase/php-jwt 2
        } catch (SignatureInvalidException $e) {
            // firebase/php-jwt 3+
        } catch (\RoRdb\SignatureInvalidException $e) {
            // firebase/php-jwt 2
    ...
    {
        // @codeCoverageIgnoreStart
        if (!\class_exists('RoRdb\\SimpleJWT\\JWT')) {
    ...
    private function setPhpsecConstants()
    {
        if (\filter_var(\getenv('GAE_VM'), \FILTER_VALIDATE_BOOLEAN)) {
            ...
            if (!\defined('RoRdb\\CRYPT_RSA_MODE')) {
                \define('RoRdb\\CRYPT_RSA_MODE', RSA::MODE_OPENSSL);
            }
        }
    }
    /**
    ...
    {
        $class = \class_exists('RoRdb\\Firebase\\JWT\\JWT') ? 'Firebase\\JWT\\JWT' : 'JWT';
