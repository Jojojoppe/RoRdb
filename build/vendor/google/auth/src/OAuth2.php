...
use RoRdb\GuzzleHttp\Psr7\Query;
use RoRdb\GuzzleHttp\Psr7\Request;
use RoRdb\GuzzleHttp\Psr7\Utils;
use InvalidArgumentException;
use RoRdb\Psr\Http\Message\RequestInterface;
use RoRdb\Psr\Http\Message\ResponseInterface;
use RoRdb\Psr\Http\Message\UriInterface;
/**
...
class OAuth2 implements FetchAuthTokenInterface
{
    ...
    private function jwtDecode($idToken, $publicKey, $allowedAlgs)
    {
        if (\class_exists('RoRdb\\Firebase\\JWT\\JWT')) {
            return \RoRdb\Firebase\JWT\JWT::decode($idToken, $publicKey, $allowedAlgs);
        }
        return \RoRdb\JWT::decode($idToken, $publicKey, $allowedAlgs);
    }
    private function jwtEncode($assertion, $signingKey, $signingAlgorithm, $signingKeyId = null)
    {
        if (\class_exists('RoRdb\\Firebase\\JWT\\JWT')) {
            return \RoRdb\Firebase\JWT\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
        }
        return \RoRdb\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
    }
    /**
