 ...
 */
namespace RoRdb\Google\AccessToken;

use RoRdb\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use RoRdb\Firebase\JWT\SignatureInvalidException;
use RoRdb\GuzzleHttp\Client;
use RoRdb\GuzzleHttp\ClientInterface;
use RoRdb\phpseclib3\Crypt\PublicKeyLoader;
use RoRdb\phpseclib3\Crypt\RSA\PublicKey;
use RoRdb\Psr\Cache\CacheItemPoolInterface;
use RoRdb\Google\Auth\Cache\MemoryCacheItemPool;
use RoRdb\Google\Exception as GoogleException;
use RoRdb\Stash\Driver\FileSystem;
use RoRdb\Stash\Pool;
use DateTime;
use DomainException;
use Exception;
use RoRdb\ExpiredException;
// Firebase v2
use LogicException;
/**
...
class Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('RoRdb\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'RoRdb\\Firebase\\JWT\\JWT';
        }
    ...
    private function getPublicKey($cert)
    {
        ...
        if (\class_exists('RoRdb\\phpseclib3\\Crypt\\RSA\\PublicKey')) {
    ...
    {
        if (\class_exists('RoRdb\\phpseclib3\\Crypt\\RSA')) {
            return 'RoRdb\\phpseclib3\\Crypt\\RSA';
        }
        if (\class_exists('RoRdb\\phpseclib\\Crypt\\RSA')) {
            return 'RoRdb\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('RoRdb\\phpseclib3\\Math\\BigInteger')) {
            return 'RoRdb\\phpseclib3\\Math\\BigInteger';
        }
        if (\class_exists('RoRdb\\phpseclib\\Math\\BigInteger')) {
            return 'RoRdb\\phpseclib\\Math\\BigInteger';
        }
        return 'Math_BigInteger';
    }
    private function getOpenSslConstant()
    {
        if (\class_exists('RoRdb\\phpseclib3\\Crypt\\AES')) {
            return 'phpseclib3\\Crypt\\AES::ENGINE_OPENSSL';
        }
        if (\class_exists('RoRdb\\phpseclib\\Crypt\\RSA')) {
            return 'phpseclib\\Crypt\\RSA::MODE_OPENSSL';
        }
        if (\class_exists('RoRdb\\Crypt_RSA')) {
    ...
    private function setPhpsecConstants()
    {
        if (\filter_var(\getenv('GAE_VM'), \FILTER_VALIDATE_BOOLEAN)) {
            if (!\defined('RoRdb\\MATH_BIGINTEGER_OPENSSL_ENABLED')) {
                \define('RoRdb\\MATH_BIGINTEGER_OPENSSL_ENABLED', \true);
            }
            if (!\defined('RoRdb\\CRYPT_RSA_MODE')) {
                \define('RoRdb\\CRYPT_RSA_MODE', \constant($this->getOpenSslConstant()));
            }
        }
    }
}
