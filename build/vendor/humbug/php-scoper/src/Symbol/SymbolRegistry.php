...
use function ltrim;
use function RoRdb\Safe\array_flip;
use function RoRdb\Safe\preg_match;
...
final class SymbolRegistry
{
    ...
    {
        return new self(array_map(static fn(string $name) => strtolower(trim($name, '\\')), $names), array_unique($regexes, SORT_STRING), \false);
    }
    /**
    ...
    {
        return new self(array_map(static fn(string $name) => self::lowerCaseConstantName(trim($name, '\\')), $names), array_unique($regexes, SORT_STRING), \true);
    }
    /**
     * @param list<string> $names
     * @param list<string> $regexes
     */
    private function __construct(array $names, array $regexes, bool $constants)
    {
