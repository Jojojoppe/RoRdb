 ...
 */
namespace RoRdb\Humbug\PhpScoper\Autoload;
...
EOF;
    ...
    public function dump() : string
    {
        ...
        $statements = implode(self::$eol, self::createClassAliasStatementsSection($this->registry->getRecordedClasses(), $hasNamespacedFunctions)) . self::$eol . self::$eol;
        $statements .= implode(self::$eol, self::createFunctionAliasStatements($exposedFunctions, $hasNamespacedFunctions));
...
PHP;
     ...
     * @return list<string>
     */
    private static function createClassAliasStatementsSection(array $exposedClasses, bool $hasNamespacedFunctions) : array
    {
    ...
    {
        return array_map(static fn(array $pair) => self::createClassAliasStatement(...$pair), $exposedClasses);
    }
    private static function createClassAliasStatement(string $original, string $alias) : string
    {
        return sprintf(<<<'PHP'
...
, $original, $alias);
     ...
     * @return list<string>
     */
    private static function createFunctionAliasStatements(array $exposedFunctions, bool $hasNamespacedFunctions) : array
    {
        $statements = array_map(static fn(array $pair) => self::createFunctionAliasStatement($hasNamespacedFunctions, ...$pair), $exposedFunctions);
        if ([] === $statements) {
            return $statements;
        }
    ...
    private static function createFunctionAliasStatement(bool $hasNamespacedFunctions, string $original, string $alias) : string
    {
        if (!$hasNamespacedFunctions) {
            return sprintf(<<<'PHP'
...
PHP
, $original, '__autoload' === $original ? '$className' : '', $alias);
        }
...
PHP
, null === $namespace ? '' : $namespace->toString() . ' ', $original, $functionName, '__autoload' === $functionName ? '$className' : '', $alias);
    }
    /**
