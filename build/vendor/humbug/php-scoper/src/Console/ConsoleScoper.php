...
use RoRdb\Humbug\PhpScoper\Autoload\ScoperAutoloadGenerator;
use RoRdb\Humbug\PhpScoper\Configuration\Configuration;
use RoRdb\Humbug\PhpScoper\Scoper\Scoper;
use RoRdb\Humbug\PhpScoper\Scoper\ScoperFactory;
use RoRdb\Humbug\PhpScoper\Symbol\SymbolsRegistry;
use RoRdb\Humbug\PhpScoper\Throwable\Exception\ParsingException;
...
use function count;
use function RoRdb\Humbug\PhpScoper\get_common_path;
use function preg_match as native_preg_match;
use function RoRdb\Safe\file_get_contents;
use function RoRdb\Safe\sprintf;
use function RoRdb\Safe\usort;
...
final class ConsoleScoper
{
    ...
    public function __construct(Filesystem $fileSystem, Application $application, ScoperFactory $scoperFactory)
    {
    ...
    private function scopeFiles(Configuration $config, string $outputDir, bool $stopOnFailure, ScoperLogger $logger) : void
    {
        ...
        $vendorDir = self::findVendorDir([...array_column($files, 2), ...array_column($whitelistedFiles, 2)]);
        if (null !== $vendorDir) {
    ...
    private static function findVendorDir(array $outputFilePaths) : ?string
    {
        ...
        usort($vendorDirs, static fn($a, $b) => strlen($a) <=> strlen($b));
