 ...
 */
namespace RoRdb\Humbug\PhpScoper\Console;

use RoRdb\Fidry\Console\Application\Application as FidryApplication;
use RoRdb\Humbug\PhpScoper\Console\Command\AddPrefixCommand;
use RoRdb\Humbug\PhpScoper\Console\Command\InitCommand;
use RoRdb\Humbug\PhpScoper\Console\Command\InspectSymbolCommand;
use RoRdb\Humbug\PhpScoper\Container;
use RoRdb\Symfony\Component\Console\Helper\FormatterHelper;
use function RoRdb\Humbug\PhpScoper\get_php_scoper_version;
...
ASCII;
    ...
    {
        return new self(new Container(), get_php_scoper_version(), \false === strpos(self::RELEASE_DATE_PLACEHOLDER, '@') ? self::RELEASE_DATE_PLACEHOLDER : '', \true, \true);
    }
    public function __construct(Container $container, string $version, string $releaseDate, bool $isAutoExitEnabled, bool $areExceptionsCaught)
    {
