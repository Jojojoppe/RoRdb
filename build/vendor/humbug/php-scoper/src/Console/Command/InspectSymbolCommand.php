 ...
 */
namespace RoRdb\Humbug\PhpScoper\Console\Command;

use RoRdb\Fidry\Console\Command\Command;
use RoRdb\Fidry\Console\Command\CommandRegistry;
use RoRdb\Fidry\Console\Command\Configuration as CommandConfiguration;
use RoRdb\Fidry\Console\ExitCode;
use RoRdb\Fidry\Console\Input\IO;
use RoRdb\Humbug\PhpScoper\Configuration\Configuration;
use RoRdb\Humbug\PhpScoper\Configuration\ConfigurationFactory;
use RoRdb\Humbug\PhpScoper\Console\ConfigLoader;
use RoRdb\Humbug\PhpScoper\Symbol\EnrichedReflector;
use RoRdb\Humbug\PhpScoper\Symbol\EnrichedReflectorFactory;
use InvalidArgumentException;
use RoRdb\Symfony\Component\Console\Application as DummyApplication;
use RoRdb\Symfony\Component\Console\Input\InputArgument;
use RoRdb\Symfony\Component\Console\Input\InputOption;
use RoRdb\Symfony\Component\Console\Output\NullOutput;
use RoRdb\Symfony\Component\Filesystem\Filesystem;
use RoRdb\Symfony\Component\Filesystem\Path;
use function assert;
use function file_exists;
use function implode;
use function in_array;
use function RoRdb\Safe\getcwd;
...
final class InspectSymbolCommand implements Command
{
    ...
    public function __construct(Filesystem $fileSystem, ConfigurationFactory $configFactory, EnrichedReflectorFactory $enrichedReflectorFactory)
    {
    ...
    {
        return new CommandConfiguration('inspect-symbol', 'Checks the given symbol for a given configuration. Helpful to have an insight on how PHP-Scoper will interpret this symbol', '', [new InputArgument(self::SYMBOL_ARG, InputArgument::REQUIRED, 'The symbol to inspect.'), new InputArgument(self::SYMBOL_TYPE_ARG, InputArgument::OPTIONAL, sprintf('The symbol type inspect ("%s").', implode('", "', SymbolType::ALL)), SymbolType::ANY_TYPE)], [ChangeableDirectory::createOption(), new InputOption(self::CONFIG_FILE_OPT, 'c', InputOption::VALUE_REQUIRED, sprintf('Configuration file. Will use "%s" if found by default.', ConfigurationFactory::DEFAULT_FILE_NAME)), new InputOption(self::NO_CONFIG_OPT, null, InputOption::VALUE_NONE, 'Do not look for a configuration file.')]);
    }
    ...
    private static function getSymbolType(IO $io) : string
    {
        ...
        if (!in_array($type, SymbolType::ALL, \true)) {
            throw new InvalidArgumentException(sprintf('Expected symbol type to be one of "%s". Got "%s"', implode('", "', SymbolType::ALL), $type));
        }
        return $type;
    }
     ...
     */
    private static function printSymbol(IO $io, string $symbol, string $type, ?string $configPath, EnrichedReflector $reflector) : void
    {
     ...
     * @param SymbolType::*_TYPE $type
     */
    private static function printInspectionHeadline(IO $io, string $symbol, string $type) : void
    {
        $io->writeln(sprintf('Inspecting the symbol <comment>%s</comment> %s', $symbol, SymbolType::ANY_TYPE === $type ? 'for all types.' : sprintf('for type <comment>%s</comment>:', $type)));
    }
    private static function printAnyTypeSymbol(IO $io, string $symbol, EnrichedReflector $reflector) : void
    {
        foreach (SymbolType::getAllSpecificTypes() as $specificType) {
            $io->writeln(sprintf('As a <comment>%s</comment>:', $specificType));
     ...
     * @param SymbolType::*_TYPE $type
     */
    private static function printTypedSymbol(IO $io, string $symbol, string $type, EnrichedReflector $reflector) : void
    {
    ...
    private static function determineSymbolStatus(string $symbol, string $type, EnrichedReflector $reflector) : array
    {
        switch ($type) {
            case SymbolType::CLASS_TYPE:
                return [$reflector->isClassInternal($symbol), $reflector->isExposedClass($symbol)];
            case SymbolType::FUNCTION_TYPE:
                return [$reflector->isClassInternal($symbol), $reflector->isExposedFunction($symbol)];
            case SymbolType::CONSTANT_TYPE:
                return [$reflector->isConstantInternal($symbol), $reflector->isExposedConstant($symbol)];
        }
