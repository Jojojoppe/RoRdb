...
use RoRdb\Symfony\Component\Console\Input\StringInput;
...
final class ConfigLoader
{
    ...
    public function __construct(CommandRegistry $commandRegistry, Filesystem $fileSystem, ConfigurationFactory $configFactory)
    {
    ...
    {
        if ('' !== $prefix) {
            return $this->configFactory->createWithPrefix($config, $prefix);
        }
        return $config;
    }
    private function configurePaths(Configuration $config, string $cwd) : Configuration
    {
        ...
        if (0 === count($config->getFilesWithContents())) {
            return $this->configFactory->createWithPaths($config, [$cwd]);
        }
        return $config;
    }
    /**
