...
use RoRdb\Humbug\PhpScoper\Patcher\ComposerPatcher;
use RoRdb\Humbug\PhpScoper\Patcher\Patcher;
use RoRdb\Humbug\PhpScoper\Patcher\PatcherChain;
use RoRdb\Humbug\PhpScoper\Patcher\SymfonyPatcher;
...
use function RoRdb\Humbug\PhpScoper\chain;
...
use function RoRdb\Safe\file_get_contents;
...
final class ConfigurationFactory
{
    ...
    public function __construct(Filesystem $fileSystem, SymbolsConfigurationFactory $configurationWhitelistFactory)
    {
    ...
    public function create(?string $path = null, array $paths = []) : Configuration
    {
        ...
        return new Configuration($path, $prefix, $filesWithContents, self::retrieveFilesWithContents($excludedFiles), new PatcherChain($patchers), $symbolsConfiguration);
    }
    /**
     * @param string[] $paths
     */
    ...
    {
        $filesWithContents = self::retrieveFilesWithContents(chain(self::retrieveFilesFromPaths(array_unique($paths, SORT_STRING))));
        return new Configuration($config->getPath(), $config->getPrefix(), array_merge($config->getFilesWithContents(), $filesWithContents), $config->getExcludedFilesWithContents(), $config->getPatcher(), $config->getSymbolsConfiguration());
    }
    public function createWithPrefix(Configuration $config, string $prefix) : Configuration
    {
        ...
        return new Configuration($config->getPath(), $prefix, $config->getFilesWithContents(), $config->getExcludedFilesWithContents(), $config->getPatcher(), $config->getSymbolsConfiguration());
    }
