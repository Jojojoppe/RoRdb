 ...
 */
namespace RoRdb\phpseclib3\File;

use DateTimeImmutable;
use DateTimeInterface;
use DateTimeZone;
use RoRdb\ParagonIE\ConstantTime\Base64;
use RoRdb\ParagonIE\ConstantTime\Hex;
...
use RoRdb\phpseclib3\Crypt\RSA\Formats\Keys\PSS;
use RoRdb\phpseclib3\Exception\UnsupportedAlgorithmException;
use RoRdb\phpseclib3\File\ASN1\Element;
use RoRdb\phpseclib3\File\ASN1\Maps;
use RoRdb\phpseclib3\Math\BigInteger;
use RoRdb\phpseclib3\Crypt\PublicKeyLoader;
/**
 * Pure-PHP X.509 Parser
 *
 * @package X509
    ...
    private function validateSignatureCountable($caonly, $count)
    {
        ...
        switch (\true) {
            case isset($this->currentCert['tbsCertificate']):
                // self-signed cert
                switch (\true) {
                    case !\defined('RoRdb\\FILE_X509_IGNORE_TYPE') && $this->currentCert['tbsCertificate']['issuer'] === $this->currentCert['tbsCertificate']['subject']:
                    case \defined('RoRdb\\FILE_X509_IGNORE_TYPE') && $this->getIssuerDN(self::DN_STRING) === $this->getDN(self::DN_STRING):
                ...
                if (!empty($this->CAs)) {
                    for ($i = 0; $i < \count($this->CAs); $i++) {
                        ...
                        $ca = $this->CAs[$i];
                        switch (\true) {
                            case !\defined('RoRdb\\FILE_X509_IGNORE_TYPE') && $this->currentCert['tbsCertificate']['issuer'] === $ca['tbsCertificate']['subject']:
                            case \defined('RoRdb\\FILE_X509_IGNORE_TYPE') && $this->getDN(self::DN_STRING, $this->currentCert['tbsCertificate']['issuer']) === $this->getDN(self::DN_STRING, $ca['tbsCertificate']['subject']):
                                $authorityKey = $this->getExtension('id-ce-authorityKeyIdentifier');
            ...
            case isset($this->currentCert['tbsCertList']):
                if (!empty($this->CAs)) {
                    for ($i = 0; $i < \count($this->CAs); $i++) {
                        $ca = $this->CAs[$i];
                        switch (\true) {
                            case !\defined('RoRdb\\FILE_X509_IGNORE_TYPE') && $this->currentCert['tbsCertList']['issuer'] === $ca['tbsCertificate']['subject']:
                            case \defined('RoRdb\\FILE_X509_IGNORE_TYPE') && $this->getDN(self::DN_STRING, $this->currentCert['tbsCertList']['issuer']) === $this->getDN(self::DN_STRING, $ca['tbsCertificate']['subject']):
                                $authorityKey = $this->getExtension('id-ce-authorityKeyIdentifier');
                                $subjectKeyID = $this->getExtension('id-ce-subjectKeyIdentifier', $ca);
                                switch (\true) {
                                    case !\is_array($authorityKey):
