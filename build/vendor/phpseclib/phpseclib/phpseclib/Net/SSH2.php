 ...
 */
namespace RoRdb\phpseclib3\Net;

use RoRdb\phpseclib3\Crypt\Blowfish;
use RoRdb\phpseclib3\Crypt\Common\SymmetricKey;
use RoRdb\phpseclib3\Crypt\Hash;
use RoRdb\phpseclib3\Crypt\Random;
use RoRdb\phpseclib3\Crypt\RC4;
use RoRdb\phpseclib3\Crypt\Rijndael;
use RoRdb\phpseclib3\Crypt\Common\PublicKey;
use RoRdb\phpseclib3\Crypt\Common\PrivateKey;
use RoRdb\phpseclib3\Crypt\RSA;
use RoRdb\phpseclib3\Crypt\DSA;
use RoRdb\phpseclib3\Crypt\EC;
use RoRdb\phpseclib3\Crypt\DH;
use RoRdb\phpseclib3\Crypt\TripleDES;
use RoRdb\phpseclib3\Crypt\Twofish;
use RoRdb\phpseclib3\Crypt\ChaCha20;
use RoRdb\phpseclib3\Math\BigInteger;
// Used to do Diffie-Hellman key exchange and DSA/RSA signature verification.
use RoRdb\phpseclib3\System\SSH\Agent;
use RoRdb\phpseclib3\System\SSH\Agent\Identity as AgentIdentity;
use RoRdb\phpseclib3\Exception\NoSupportedAlgorithmsException;
use RoRdb\phpseclib3\Exception\UnsupportedAlgorithmException;
use RoRdb\phpseclib3\Exception\UnsupportedCurveException;
use RoRdb\phpseclib3\Exception\ConnectionClosedException;
use RoRdb\phpseclib3\Exception\UnableToConnectException;
use RoRdb\phpseclib3\Exception\InsufficientSetupException;
use RoRdb\phpseclib3\Common\Functions\Strings;
use RoRdb\phpseclib3\Crypt\Common\AsymmetricKey;
/**
 * Pure-PHP implementation of SSHv2.
 *
 * @package SSH2
...
class SSH2
{
    ...
    private function connect()
    {
        ...
        $extra = $matches[1];
        if (\defined('RoRdb\\NET_SSH2_LOGGING')) {
    ...
    private function get_binary_packet($skip_channel_filter = \false)
    {
        ...
        if ($this->decrypt) {
            switch ($this->decryptName) {
                case 'aes128-gcm@openssh.com':
                case 'aes256-gcm@openssh.com':
                    $this->decrypt->setNonce($this->decrypt->fixed . $this->decryptInvocationCounter);
    ...
    protected function send_binary_packet($data, $logged = null)
    {
        ...
        if ($this->encrypt) {
            switch ($this->encryptName) {
                case 'aes128-gcm@openssh.com':
                case 'aes256-gcm@openssh.com':
                    $this->encrypt->setNonce($this->encrypt->fixed . $this->encryptInvocationCounter);
    ...
    {
        if (\defined('RoRdb\\NET_SSH2_LOGGING') && NET_SSH2_LOGGING == self::LOG_COMPLEX) {
